...
1. Bundled Folder Strategy: Checks if a dedicated folder exists for this client/invoice type.
...
class InvoiceAssetResolver:
    ...
    def resolve_assets_for_input_file(self, input_file_path: str) -> Optional[InvoiceAssets]:
        ...
        # 1. Try to find a "Bundled" configuration (Self-contained folder)
        #    Example: config/bundled/JF25058_config/
        bundled_assets = self._try_resolve_from_bundled_folder(stem)
        if bundled_assets:
            logger.info(f"✅ Resolved assets using Bundled Strategy: {bundled_assets.config_path.parent.name}")
            return InvoiceAssets(input_path, bundled_assets.config_path, bundled_assets.template_path)
        ...
        # 3. Default Fallback
        #    If nothing specific found, use 'default.json' and 'Invoice.xlsx' if available.
    ...
    def _try_resolve_from_bundled_folder(self, file_stem: str) -> Optional[InvoiceAssets]:
        """
        Strategy 1: Look for a dedicated folder for this configuration.
        Expected structure: .../config/bundled/{Stem}_config/
        """
        ...
        for candidate in candidates:
            ...
            # Direct check
            folder_name = f"{candidate}_config"
    ...
    def _get_assets_from_folder(self, folder_path: Path, identifier: str) -> Optional[InvoiceAssets]:
        ...
        for f in folder_path.iterdir():
            if f.suffix.lower() == '.json':
                ...
                if "template_config" not in f.name:
                    config_file = f
    ...
    def _resolve_fallback(self) -> Optional[InvoiceAssets]:
        """Strategy 3: The 'safety net' default assets."""
        default_config = self.config_dir / "default.json"
        default_template = self.template_dir / "Invoice.xlsx"
        
        if default_config.exists() and default_template.exists():
             return InvoiceAssets(Path(""), default_config, default_template)
        return None

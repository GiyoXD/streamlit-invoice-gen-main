...
from .template_analyzer import TemplateAnalysisResult, SheetAnalysis, ColumnInfo
...
class BundleBuilder:
    ...
    # Default footer sum columns by sheet type
    FOOTER_SUM_COLUMNS = {
    ...
    def build_bundle(self, analysis: TemplateAnalysisResult) -> Dict[str, Any]:
        """
        Build complete bundle config from template analysis.
        
        Args:
            analysis: TemplateAnalysisResult from TemplateAnalyzer
            
        Returns:
            Complete bundle config dictionary
        """
        self.logger.info(f"Building bundle config for {analysis.customer_code}")
    ...
    def _build_meta(self, analysis: TemplateAnalysisResult) -> Dict[str, Any]:
        """Build _meta section."""
        return {
            "config_version": "2.1_auto_generated",
            "customer": analysis.customer_code,
            "created": datetime.now().strftime("%Y-%m-%d"),
            "description": f"Auto-generated config for {analysis.customer_code}",
            "source_template": analysis.file_path,
            "generator": "auto_bundle_generator"
        }
    ...
    def _build_footer(self, sheet: SheetAnalysis) -> Dict[str, Any]:
        ...
        sum_cols = []
        default_sum = self.FOOTER_SUM_COLUMNS.get(sheet.data_source, [])
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python bundle_builder.py <template.xlsx>")
        sys.exit(1)
    ...
    bundle = builder.build_bundle(result)

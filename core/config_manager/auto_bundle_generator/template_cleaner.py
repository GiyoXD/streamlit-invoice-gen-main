...
from .template_analyzer import TemplateAnalysisResult, SheetAnalysis

logger = logging.getLogger(__name__)

class TemplateCleaner:
    """Cleans raw Excel files to create reusable templates."""
    ...
    def clean_template(self, workbook: openpyxl.Workbook, analysis: TemplateAnalysisResult) -> Tuple[openpyxl.Workbook, Dict[str, Any]]:
        """
    ...
    def _clean_sheet(self, ws: Worksheet, analysis: SheetAnalysis) -> Dict[str, Any]:
        ...
        if not footer_start_row:
             error_msg = f"Footer not detected in {analysis.name}! Could not find a row containing 'Total' and '=SUM'."
             self.logger.error(error_msg)
             # Alert user / Safety Stop
    ...
    def _find_footer_start(self, ws: Worksheet, search_start_row: int) -> Optional[int]:
        ...
        - Look for a row that has BOTH:
            1. 'total' (case-insensitive) in any cell
            2. Formula starting with '=SUM' (case-insensitive) in any cell
        """
        # Scan backwards
